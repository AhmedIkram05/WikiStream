...
**Evidence:** `uv run --project gx pytest -q --tb=short -m ch tests/gx` → 2 passed; suite vs live CH → skipped:true on empty hour, failing path as above. VM scheduled green run = AC10 (pending, 4.2.5).
...
**Status:** IN PROGRESS
**2026-08-13:** Local evidence complete (suite failing-path proof, JSON contract, empty-window skip). VM steps pending Ahmed's commit/PR/merge: `systemctl show wikistream-gx.service -p ExecMainStatus` = 0 AND `/var/log/wikistream-gx.log` has a matching `success: true` JSON line on a scheduled :30 run (AC10); one failing-path demo (point at bad window → non-zero exit + `success: false` → restore config); AC11 recorded on VM.
...
**Status:** IN PROGRESS
**2026-08-13:** Procedure proven locally (probe): `RESTORE DATABASE default AS restore_check FROM Disk('backups','<name>')` → counts equal, full object set present, `DROP DATABASE restore_check` (no residue). **Deviation:** restore runs as the container **default** user, not wikistream — wikistream has `CREATE DATABASE ON default.*` only and RESTORE-as-wikistream fails Code 497 (needs CREATE DATABASE ON restore_check.*); deliberate least-privilege, no grant added. The full four-step spot-check (GCS lift-back → restore → equality numbers → sample-row match → drop) is executed on the VM after merge and recorded here (AC16).

### 4.2.9 — 4B PR → deploy → verification battery → Gate 2 record → log + coverage boundary
**Status:** IN PROGRESS
**2026-08-13:** Local leg complete: `pre-commit run --all-files` all 8 hooks green; `uv run --project consumer pytest -m "not ch"` 67 passed; consumer ch 27 passed (`--ignore=tests/gx`); gx ch 2 passed; full stack healthy×3; backup regression green; failing-path suite proof. Coverage boundary corrected (healthcheck.py + gx/suite.py real with suites; backup.sh follows the export/parity wrapper convention, outside the pytest-cov gate, verified by production timer runs + the restore spot-check). VM leg after Ahmed's merge: verification battery (AC13 docker ps healthy×3 + inspect; AC14 KILL-proof re-run + HEALTH_STALE_SECONDS=5 stale→restart + `resumed_from=` record; systemd supervision exercised: `systemctl stop/start docker` → stack auto-recovers, timers fire via Persistent; AC15 backup cadence/lift/prune live; AC16 restore spot-check recorded), then **Gate 2 record** — kill-recovery ✅ (4A + 4B), malformed→dead_letter ✅ (4A), backup restore ✅ (4B), healthchecks exercised ✅ (4B); GO-with-caveat recorded (caveat = GX enforcement deferred to Phase 6 by plan). Handoff §10 recorded in wrap-up.
...
**Evidence:** reviewer report (11 prioritized findings, only blocker was apply.yml context — fixed), post-fix battery rerun green, reproduced+fixed plan diff for the attached_disk artifact.
