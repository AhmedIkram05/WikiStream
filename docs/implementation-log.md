...
**2026-08-14:**
1. **PR merged & applied** — 5C changes (iam-review.md, network.tf null_resource, main.tf null-provider declaration, lockfile, log) committed on `feature/Observability-&-Security-Hardening` (`50329c4`), working tree clean. Ahmed ran `terraform apply`; the null_resource's local-exec deleted all 4 `default-allow-*` rules (idempotent `|| true`). Console/OS Login access is not firewall-governed, so the lockdown cannot lock out the operator.
2. **Rule-state verification (orchestrator, live read-only):**
   - `gcloud compute firewall-rules list` → **exactly 4 custom rules, zero `default-allow-*`**: allow-clickhouse `8123`, allow-grafana `3000`, allow-ssh `22` (all from `209.35.91.152/32`), allow-internal `10.0.0.0/24`.
   - `gcloud compute firewall-rules describe default-allow-ssh` → **NOT_FOUND** ("The resource 'projects/wikistream-505003/global/firewalls/default-allow-ssh' was not found").
3. **Live probes (Ahmed, Cloud Shell vs home):** curl to `:3000`/`:8123` from Cloud Shell → refused; ssh → refused; home-IP positive control → succeeds. Non-whitelisted IP rejected.
4. **Go/No-Go (Phase 5, master plan §5):** **GO-with-caveat against AC1–AC18.** AC15 (iam-review.md committed; matrix rows cross-check fresh `gcloud get-iam-policy` + `bq show`, no phantom bindings) and AC16 (default rules gone; non-whitelisted IP rejected) fully evidenced above. Caveat: AC7–AC14/AC17/AC18 evidence lands in the DEMO battery, which runs immediately after in the same session (§6 — no separate booking); Gate 2's GO-with-caveat already on record (§4.2.9). No-Go conditions: none.

**Evidence:** `gcloud compute firewall-rules list` (4 custom rows, 0 default) + `describe default-allow-ssh` NOT_FOUND, 2026-08-14; Ahmed's Cloud Shell/home probe results; merge commit `50329c4`.

### DEMO — Chaos battery (one ~90 min window, runs immediately after 5C — no separate booking, Ahmed 2026-08-13)

**Status:** DONE

**2026-08-14 (ran 00:59–02:20Z, immediately after the 5C.3 deploy):**
1. **DEVIATION-DEMO-1:** the orchestrator drove the battery (SSH→VM + gcloud, per Ahmed "cant you do this DEMO battery?"); Ahmed covered the human legs — Slack screenshots, email receipts, Cloud Shell negative probe. All 8 injections executed exactly per §6.DEMO, each verified, each restored, each cleared.
2. **DEMO #1 — R1 consumer-down:** `docker stop wikistream-consumer` 00:59:40Z → **R1 firing 01:01:37/01:02:12** → `docker start` 01:02:38 → heartbeats resumed 01:04:00–01:04:30 → **R1 inactive 01:04:41**, container healthy. AC7 ✓.
3. **DEMO #2 — R2 dead-letter-rate:** **DEVIATION-DEMO-2:** the 4A sse-fixture lived outside the repo; reconstructed as `/tmp/sse_fixture.py` on the VM (python HTTP server on 8787 streaming `data: {"this is":"not valid json"` frames @100/s, no `id:` fields so the durable cursor was untouched). Consumer override `docker run --name wikistream-consumer-demo2` with `STREAM_URL=http://sse-fixture:8787`. dead_letter 8,211 rows in 5 min, all `validation:invalid_json`; heartbeats inserted_delta=0 / dead_lettered_delta=298–476 per 15s (ratio → 1.0) → **R2 pending 01:11:47, FIRING 01:14:30** (Slack receipt confirmed by Ahmed) → teardown 01:18, main consumer resumed from the same checkpoint (durable state untouched) → **R2 cleared ~01:24** as the 5-min window rolled past. AC8 ✓.
4. **DEMO #3 — R3 ClickHouse insert failures:** `docker stop wikistream-clickhouse` 01:19 (90 s) → consumer log `insert_failed=182000`, `flush_failed reason=Network Error: Cannot connect to host clickhouse:8123` (resilient, kept processing, no crash) → mid-outage probe: **R1 co-firing** (heartbeats stop), R3 pending (R4/R5 evals errored — datasource-down artifact) → `docker start` → consumer flushed the backlog (insert_failed deltas post-restore) → **R3 FIRING 01:24:00**, **R1 inactive**, DLQ inactive → **R3 cleared ~01:26**. AC9 ✓ (R1 co-fire evidenced).
5. **DEMO #4 — R4 parity-drift:** `bq query DELETE` on all 4 BQ tables for window 00:00–01:00 (pre: kpi_edits 480, kpi_top_pages 226,936, kpi_edit_sizes 6, raw_events_sample 210,887 → post: all 0) → **DEVIATION-DEMO-3 (defect found):** `parity.sh` verdict write FAILED with `Code: 48 ... Processing async inserts with both inlined and external data (from stdin or infile) is not supported` — `docker exec -i` + inline VALUES + server `async_insert=1`; interactive stdin → verdict never landed → the R4 alert-loop was **dead on real drift** (systemd runs were immune via /dev/null stdin). **Fix (repo + VM, verified):** drop `-i` on the verdict-write line only (`warehouse/parity.sh` line 225); the two CH compare lines (152/166) keep `-i` (they feed SQL via here-strings). After the fix, parity reported real drift (CH 902,762 edits / 210,887 raw vs BQ 0) → verdict **0.0** landed 01:31:41/01:37:02/01:39:32 → **R4 FIRING 01:40:34** → restore: `export.sh` (01:40:51→01:45:22) reloaded the identical window (rows_edits=480, top_pages=226,936, sizes=6, raw_sample=210,887) → `parity.sh` → freshness ok, all tables ok → verdict **1.0** 01:47:37 → **R4 inactive 01:48:16**. AC10 ✓.
6. **DEMO #5 — R5 GX suite failed (forced-fail):** `docker run --rm ... -e GX_ROW_MIN=999999999 <gx image>` → `{"success": false, ... "expectations_passed": 0, "expectations_failed": 1, "row_count": 2118263, "error": "row_count 2118263 outside [999999999, 5000000]"}` → verdict **0** written 01:52:02, exit 1. AC11 ✓ (forced-fail exits non-zero AND writes 0.0 — AC4 leg).
7. **DEMO #6 — CM disk-almost-full:** pre-injection 50% (24G/24G). `dd if=/dev/zero of=/mnt/ch-data/demo_dummy bs=1M count=17000 conv=fsync` 01:53:27 (17 GiB, 173.84 s) → 86% (41G used/6.6G free) → raw `agent.googleapis.com/disk/percent_used` (device=/dev/sdb, state=used): 70.7→81.9→82.0→81.9→82.1→82.4→82.3→82.3 (01:55–02:02) — **sustained >80% for 7+ min**, condition (GT 80, 5-min duration) met ~02:01–02:04 → email notification (Ahmed's leg) → `rm demo_dummy` → **51%** (24G used/23G free) back to baseline. AC13 ✓ (AC12 leg: Ops Agent active — `systemctl is-active google-cloud-ops-agent*` — and disk/percent_used series present for ch-data).
8. **DEMO #7 — CM VM unreachable:** `gcloud compute instances stop wikistream-vm` 02:08:49Z → `compute.googleapis.com/instance/uptime` series **absent from 02:08** (last point 46.7s partial) → condition (uptime absent 120 s) met ~02:10 → email (Ahmed's leg) → `gcloud compute instances start` 02:12:44 (IP unchanged) → boot.sh brought the stack back: **all 4 containers healthy by ~02:18** (clickhouse 5 min, consumer 3 min healthy, grafana 2 min, gx Up), heartbeat 10 s old, grafana api/health ok v13.1.1 → uptime series resumed (60 s points 02:14–02:16) → **R1/R2/R3/R4 all inactive**, R5 pending (re-evaluating post-boot; fire evidence already captured). AC14 ✓.
9. **DEMO #8 — firewall reject/allow:** negative = Ahmed's Cloud Shell probes (`:3000`/`:8123`/ssh refused — 5C.3 evidence); positive control from whitelisted egress 209.35.91.152: `curl http://34.148.138.220:3000/api/health` → **HTTP 200** (0.106 s), `http://34.148.138.220:8123/ping` → **HTTP 200** (0.233 s) — allowed exactly per allow-grafana/allow-clickhouse 209.35.91.152/32. AC16 completed with both directions evidenced.
10. **Findings recorded (pre-existing, surfaced by the battery):**
    - **Finding 1 (R5 was already firing at baseline — correct alert, not artifact):** GX suite caught genuine data-lag drift — the consumer replays from a checkpoint ~6.8–9.1 h behind wall clock (checkpoint ~2026-08-13 18:11); median `lag_seconds` steady ~32,818 s over the 23–00 window; 0 of 733,841 recent inserts had event_timestamp in the last hour; not draining (transient catch-up, expected to clear over hours). The alert did its job. Cosmetic CH note: `formatDateTime(ts,'%H:%M:%S')` renders garbage on this build — use `toUnixTimestamp`/`toString` for readable evidence.
    - **Finding 2 (R4 was already firing at baseline — self-healed):** the 23:00–00:00 export never completed (interrupted around the 00:36 VM reboot during Ahmed's handoff) → parity gated on a missing export_runs record → no verdict → R4 fired. The 01:05 cycle completed in time and R4 cleared before the DEMO #4 injection. Marginal timing (export ~:01→:05:37–07, parity :05:06) — recommend (plan §10) nudging the parity timer after the export completes to de-risk.
11. **AC mapping:** AC7–AC14 = DEMO #1–#8 legs above (AC12 = ops-agent + disk metrics; AC13 = disk email; AC14 = vm-unreachable email); **AC17: 8/8 chaos injections executed with evidence — each fired, each verified, each restored, each cleared** (timestamps + restore + clear in this entry); **AC18:** dashboards remain fresh (wikistream-live updating ≤1 min, GX suite failed pending→firing as expected post-boot) and `docker compose ps` all 4 healthy at battery end.
12. **Go/No-Go (Phase 5, master plan §5):** **GO.** All 8 DEMO legs + AC1–AC18 evidenced (AC1–AC6 from the 5A/5B entries and this battery's AC4/AC6 legs; AC7–AC14 above; AC15–AC16 from 5C.1/5C.3; AC17–AC18 above). No-Go conditions: none — the parity write defect (DEVIATION-DEMO-3) was fixed and re-verified within the battery; the R4/R5 pre-firing states were diagnosed as legitimate signal, not regression.

**Evidence:** Grafana alert-state polls (per-rule firing/inactive with timestamps above); pipeline_health parity verdict rows (0/drift 01:31:41, 01:37:02, 01:39:32 → 1/ok 01:47:37) and gx verdict rows (0/row-count error 01:52:02; 0/median-lag 01:33); dead_letter count + reason histogram (`validation:invalid_json`); consumer logs (`insert_failed=182000`, flush_failed Network Error); export.sh reload log; `df -h /mnt/ch-data` before/after (50% → 86% → 51%); Monitoring REST timeSeries raw points (disk percent_used /dev/sdb state=used; instance/uptime absent 02:08→02:12); `curl` positive-control HTTP 200 on :3000/:8123 from 209.35.91.152; Ahmed's Slack receipts (R2) and email receipts (R6/R7); repo fix `warehouse/parity.sh` line 225 (`docker exec -i` → `docker exec` on the verdict write only) — mirrored on the VM copy and verified.
