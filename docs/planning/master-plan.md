...
- **IP-lockout risk from Phase 5's firewall rule.** If Ahmed's IP changes (dynamic IP, different network) after the firewall is locked down, Phases 6–8 could lose access to Grafana/ClickHouse. Worth either using a stable IP/small allowlist buffer, or sequencing the firewall lockdown after most hands-on evidence work is done.
