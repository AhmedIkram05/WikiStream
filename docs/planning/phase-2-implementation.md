...
| Artifact Registry repo; VM SA granted scoped `artifactregistry.reader` (ADR-008) | Versioned migrations / full schema / MVs / TTL (Phase 3A) |
...
| # | Decision |
| --- | --- |
| Q1 | **Bootstrap owns identity.** `infra/bootstrap` (local state, one manual apply) is extended beyond the state bucket to also create: project API enablement, the WIF pool + GitHub provider, the `wikistream-deploy` SA, and the SA↔pool binding. **Recorded deviation from ADR-007's "bootstrap = bucket only" wording** — identity has the same "can't bootstrap itself" property as the bucket; CI works from the very first PR with zero further manual steps. Logged in implementation-log as a deviation. |
| Q2 | **VM provisioning = Terraform startup script; deploy = `gcloud compute instances reset`.** The script (in-repo template, `infra/main/templates/startup.sh`) installs Docker, `git clone --depth 1` the public repo (single source of truth for compose + grafana provisioning — no duplicated files), renders `001-init.sql` from the spike-verified recipe with the Secret Manager password (see Q9), writes `.env` from secrets, `gcloud auth configure-docker`, `docker compose pull && up -d --no-build`. Runs on every boot. The apply workflow's final step resets the VM → startup re-runs → new image live. No SSH from CI — the deploy SA has zero SSH rights. |
| Q3 | **Two secrets now, auto-generated:** `clickhouse-password` + `grafana-admin-password`, created by Terraform via `random_password` (`special = false` — alphanumeric avoids shell-escaping in the startup script; length 24). Zero plaintext anywhere. `STREAM_URL`/`USER_AGENT` stay non-secret env. |
...
| `ubuntu-os-cloud/ubuntu-2404-lts` image; `docker.io` + `docker-compose-plugin` packages exist in noble repos | ✅ known; verify at build |
| `gcloud auth configure-docker <region>-docker.pkg.dev` (regional form) for AR pulls on the VM | ✅ standard; verify at build |
| OS Login: instance metadata `enable-oslogin = "TRUE"`; requires `oslogin.googleapis.com` API | ✅ known |
...
infra/main/
  ...
  modules/
    ...
    storage/         # Artifact Registry repo + scoped artifactregistry.reader for VM SA
...
- `google_project_service` for: `compute.googleapis.com`, `artifactregistry.googleapis.com`, `secretmanager.googleapis.com`, `iamcredentials.googleapis.com`, `oslogin.googleapis.com`, `cloudresourcemanager.googleapis.com` (project readiness in one manual step so CI's first run needs nothing).
...
`gcloud iam workload-identity-pools describe wikistream-ci`. Record outputs +
deviation in log (task 2.8).
...
**`modules/storage`:** `google_artifact_registry_repository` `wikistream-consumer`
(location `us-central1`, format `DOCKER`) + `google_artifact_registry_repository_iam_member`
`roles/artifactregistry.reader` for the VM SA — **scoped to that one repository**
(ADR-008's named deliverable; nothing broader).
...
- `metadata_startup_script` = file `templates/startup.sh`; `metadata = { enable-oslogin = "TRUE" }` (Q5).
...
#!/usr/bin/env bash
set -euo pipefail
exec >> /var/log/wikistream-startup.log 2>&1
...
# 2. Repo = single source of truth for compose + grafana provisioning
rm -rf /opt/wikistream && git clone --depth 1 https://github.com/AhmedIkram05/WikiStream /opt/wikistream
cd /opt/wikistream

# 3. Secrets from Secret Manager (VM SA has scoped secretAccessor)
...
# 4. Render initdb.d (Q9): spike-verified recipe, real password
...
# 5. .env for compose (the Phase 1 ${VAR:-default} seams, now real)
cat > .env <<EOF
...
# 6. Pull + start. --no-build: image comes from AR, never rebuilt on the VM.
...
Notes:
- `GCP_PROJECT` — resolved inside the script via `gcloud config get-value project` or metadata; avoid hardcoding (project id is in `terraform.tfvars`, not in the instance).
- The `rm -rf` + fresh clone is deliberate: every boot re-syncs repo + image, so a reset IS a deploy of current `main` + current `latest`. The startup script itself is static — it's Terraform-owned config; deploys don't need Terraform state churn.
- `001-init.sql` runs on first boot only (empty `ch-data` volume, per Phase 1 Q3 semantics). On resets, the volume persists — table/user already exist; `IF NOT EXISTS` makes it safe regardless.
...
verification can read them back.
...
`workflow_dispatch` (Q8 re-trigger for the reapply-after-destroy). Two jobs:
...
2. `apply` (`needs: build-push`, `environment: production` — the log 0.3
   reviewer gate pauses the job here): auth → setup-terraform → `terraform
   init` → `terraform plan -out=tfplan` → `terraform apply tfplan` →
   **`gcloud compute instances reset wikistream-vm`** (Q2 deploy; deploy SA has
   `compute.admin`).
...
(you, via gcloud — the same operation the workflow runs). Confirm, with zero
manual steps: VM returns RUNNING; startup log ends with `startup done`; the
three containers come back (volume persists — data survives); consumer logs a
fresh `connected url=`, 0 Traceback; `count()` strictly increases between two
samples. This is the "VM pulls the image and reproduces Phase 1's result"
exit criterion exercised twice (initial provision + reset).
...
1. **Local destroy** (Ahmed): `gcloud config set project wikistream-505003`,
   ...
   no VM/AR repo/secrets/firewall remain (`gcloud compute instances list`,
   `gcloud artifacts repositories list`, `gcloud secrets list` all empty), and
   the bootstrap layer is UNTOUCHED: bucket, WIF pool, deploy SA all still
   present (that's why CI still works — Q1's payoff).
...
3. Re-run the core battery (AC5/AC6/AC7 abbreviated: VM RUNNING, stack up,
   consumer connected, count() increasing, Grafana 200).

### 2.8 — Wrap-up

1. Populate `docs/implementation-log.md` Phase 2 entries: 2.1 bootstrap
   extension (deviation: ADR-007 wording), 2.2–2.3 resource decisions
   (Q9/Q10 notes for Phase 5's IAM review), 2.5 gate evidence (CI run URLs,
   Waiting-state proof), 2.6 reset evidence, 2.7 destroy/reapply evidence,
   resolved WIF provider name + SA emails, IP used in `allowed_ips` (and the
   change procedure pointer), cost sanity line (e2-medium ~$23/mo + 50GB
   ~$2.5/mo + static IP $0 attached + AR/SM ~$0 → low single-digit % of the
   $300 trial over the window; formal cost note is Phase 7b).
...
| --- | --- | --- |
| AC1 | Bootstrap extended and applied | `terraform apply` in `infra/bootstrap` clean; `gcloud iam workload-identity-pools describe wikistream-ci` exists; `wikistream-deploy` SA exists; outputs recorded |
...
| AC6 | Stack live on VM | Over SSH: `docker compose ps` → 3 services Up; startup log ends `startup done`; consumer log has ≥1 `connected url=`, 0 Traceback; two `SELECT count()` samples ≥1 min apart, strictly increasing |
...
| AC8 | Secrets are real, not defaults | `gcloud secrets versions access latest --secret=grafana-admin-password` yields a 24-char value; that value logs into Grafana (proves env-only swap end-to-end); same for `clickhouse-password` (login over 8123 from laptop with it) |
| AC9 | Firewall allows exactly the allowlist | From Ahmed's IP: 22/3000/8123 reachable; ports work while the internal-only rule blocks nothing that matters (full rejection-probe testing is Phase 5 — record this boundary in the log) |
...
- **VM up but containers absent:** read the startup log — `gcloud compute ssh
  --command "cat /var/log/wikistream-startup.log"` (or serial port output if
  SSH itself fails). Common: `docker-compose-plugin` package name differs,
  `apt-get update` needed first, git clone fails (repo visibility / depth
  flag), `configure-docker` region mismatch, `.env` written after first
  `compose up` (ordering).
...
- **Cost sanity:** e2-medium ~$23/mo, 50GB pd-standard ~$2.5/mo, static IP $0
  while attached, AR/SM negligible → tens of cents/day of trial credit. If
  anything looks off, `gcloud billing` reports are the first check. (Formal
  FinOps note is Phase 7b.)
...
- **Live infrastructure to build against:** VM (static IP, OS Login),
  ClickHouse 26.3.17 + Grafana 13.1.1 + consumer image on AR, two SM secrets,
  WIF CI identity, IP allowlist firewall.
