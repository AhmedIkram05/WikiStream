...
| Single VM only, no K8s (pre-Phase 6) | GKE Autopilot **zonal** ephemeral cluster (`us-central1-a`, ≤£70/mo) — owned + torn down with WikiStream, no cross-project reference (sequential teardown) | Vertical VM was honest for P0-5 (right-sized, disposable demo). Phase 6 adds K8s for CV breadth without lying about scale: consumer stays `replicas:1` singleton (SSE `Last-Event-ID` cursor, GCS `if_generation_match`), batch timers → 4 K8s CronJobs (baked SQL), ClickHouse stays on VM (stateful tier not K8s'd). Zonal not regional — matches VM zone so pod→VM is internal IP without cross-VPC; Autopilot not free-tier, $0.10/hr management fee covered by $74 credit — see Phase 6 plan v3 slim §3/§8 and ADR-0012 v3 (2026-09-02). |
